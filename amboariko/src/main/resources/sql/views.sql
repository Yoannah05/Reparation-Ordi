...
CREATE OR REPLACE VIEW v_composants_utilises AS
SELECT 
    cu.id_cu AS id_composant_utilise,
    cu.id_rep_ordi,
    cu.id_composant,
    c.nom_composant,
    ro.id_reparation,
    r.date_debut,
    r.date_fin,
    cp.prix AS prix_composant,
    cu.quantite_utilisee,
    (cu.quantite_utilisee * cp.prix) AS total_prix
FROM Composants_Utilises cu
JOIN Composants c ON cu.id_composant = c.id_composant
JOIN Reparations_ordi ro ON cu.id_rep_ordi = ro.id_rep_ordi
JOIN Reparations r ON ro.id_reparation = r.id_reparation
JOIN ComposantsPrix cp ON cu.id_composant = cp.id_composant
WHERE cp.date <= r.date_debut;